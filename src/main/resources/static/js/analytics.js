...
var calendarHeatmap = {
    ...
    if ( !calendarHeatmap.data[0].summary ) {
      calendarHeatmap.data.map(function (d) {
        var summary = d.details.reduce( function(uniques, project) {
          if ( !uniques[project.band] ) {
            uniques[project.band] = {
              'value': project.value
            };
          } else {
            uniques[project.band].value += project.value;
          }
          return uniques;
        }, {});
    ...
    calendarHeatmap.items.selectAll('.item-circle')
      ...
      .on('mouseover', function (d) {
        ...
        for ( var i = 0; i < d.summary.length; i++ ) {
          tooltip_html += '<div><span><strong>' + d.summary[i].band + '</strong></span>';
  ...
  drawMonthOverview: function () {
    ...
    item_block.selectAll('.item-block-rect')
      ...
      .on('mouseover', function(d) {
        ...
        // Construct tooltip
        var tooltip_html = '';
        tooltip_html += '<div class="header"><strong>' + d.band + '</strong></div><br>';
  ...
  drawDayOverview: function () {
    ...
    calendarHeatmap.items.selectAll('.item-block')
      ...
      .attr('y', function (d) {
        return (projectScale(d.band) + projectScale.rangeBand() / 2) - 15;
      })
      .attr('width', function (d) {
      ...
      .on('mouseover', function(d) {
        ...
        var y = projectScale(d.band) + projectScale.rangeBand() / 2 + calendarHeatmap.settings.tooltip_padding / 2;
